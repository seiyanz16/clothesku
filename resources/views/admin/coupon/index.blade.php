                        ...
                        <tbody>
                            @if ($discountCoupons->isNotEmpty())
                                @foreach ($discountCoupons as $coupon)
                                    <tr>
                                        <td>{{ $discountCoupons->perPage() * ($discountCoupons->currentPage() - 1) + $loop->iteration }}
                                        </td>
                                        <td>{{ $coupon->code }}</td>
                                        <td>{{ $coupon->name }}</td>
                                        <td>
                                        ...
                                        <td>{{ !empty($coupon->start) ? \Carbon\Carbon::parse($coupon->start)->format('d/m/Y H:i:s') : '' }}
                                        </td>
                                        <td>{{ !empty($coupon->end) ? \Carbon\Carbon::parse($coupon->end)->format('d/m/Y H:i:s') : '' }}
                                        </td>
                                        <td>
                                            @if ($coupon->status == 1)
...
@push('scripts')
    <script>
        function deleteCoupon(id) {
            ...
            // return false;
            if (confirm("Are you sure want to delete this dicount coupon?")) {
                $.ajax({
                    url: newUrl,
                    type: 'delete',
                    data: {},
